...
- **`__all__` exports**: Explicit public API for 15 modules (`style`, `constant`, `install`, `validate`, `font`, `icon`, `cmap`, `color/_color`, `color/_conversion`, `color/_views`, `color/_loader`, `cli`, `mcp/server`, `mcp/resources`, `mcp/tools`).
...
- **Test coverage**: 228 tests, 91% line coverage. Added tests for `scale`, `io`, `layout`, `annotation`, `prompt`, `cmap`, `icon`, `font`, `constant`, `install`, `xplot`, and `asset_viz`.
...
- **`__version__`**: Synced with `pyproject.toml` (`0.1.0` → `0.1.1`).
