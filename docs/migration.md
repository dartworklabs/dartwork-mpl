...
## v5 — generative color system (0.5.6)

A 91-parameter generative system (16 families × 10 perceptually-equalized
steps, `dc.{family}{step}`) ships in 0.5.6. Its colormap catalog is the new
default; the categorical `dc.{family}{step}` remap is opt-in via
`dm.set_palette_version(5)` (see below). Design spec:
...
2. **Opt in to the v5 remap for the three colliding families** with
   ...
   keeps them frozen at the legacy value. Note: the legacy ramps run
   dark→light; the v5 ramps run light→dark, so step indices invert
   meaning under version 5 (e.g. `dc.gray0` goes from near-black to
   near-white).
...
The codemod *rewrites* prefixed tokens — a shade colour like `dc.ocean2` and
quoted `dc.` / `dm.` names like `set_cycle("dc.spectrum")`. Bare names *without*
a `dc.` / `dm.` prefix (e.g. `set_cycle("spectrum")`, `get_palette("ocean")`,
`cycle="pop"`) are now *reported* — flagged with `file:line` and the suggested
