...
The codemod matches prefixed tokens — a shade colour like `dc.ocean2` and
quoted bare names like `set_cycle("dc.spectrum")`. Bare names *without* a
`dc.` / `dm.` prefix (e.g. `set_cycle("spectrum")`, `get_palette("ocean")`)
are not detected — update those by hand.
