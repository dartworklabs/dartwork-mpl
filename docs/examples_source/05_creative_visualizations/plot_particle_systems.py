...
for i in range(n_particles):
    ...
    for j in range(trail_length - 1):
        ...
        ax.plot([trails_x[i, j], trails_x[i, j+1]],
               [trails_y[i, j], trails_y[i, j+1]],
               color=color.to_hex(), alpha=alpha, lw=0.5)

    # Draw particle
    ax.scatter(particles_x[i], particles_y[i], s=20,
              c=[color.to_hex()], edgecolors='white',
...
for wx, wy, strength in wells:
    ...
    for r, alpha in [(2, 0.1), (1.5, 0.2), (1, 0.3)]:
        circle = Circle((wx, wy), r * strength, color='white',
                       alpha=alpha, zorder=5)
        ax.add_patch(circle)
    # Core
    circle = Circle((wx, wy), 0.3, color='white',
                   edgecolor='oc.blue5', linewidth=2, zorder=15)
    ax.add_patch(circle)
...
# Title
title = ax.text(0, 6.5, 'Gravitational Particle Dynamics',
               ha='center', fontsize=dm.fs(3), color='white', weight='bold')
title.set_path_effects([path_effects.withStroke(linewidth=3, foreground='oc.blue9')])
...
for fw in range(n_fireworks):
    ...
    color_start = f'oc.{np.random.choice(["red", "blue", "green", "yellow", "purple"])}9'
    color_end = f'oc.{np.random.choice(["orange", "cyan", "pink", "teal", "violet"])}3'
    ...
    for i in range(fw_data['particles']):
        ...
        for j in range(trail_steps):
            ...
            if t_trail > 0:
                ...
                y_trail = center_y + speed * np.sin(angle) * t_trail - 0.5 * gravity * t_trail**2

                alpha = (1 - j/trail_steps) * (1 - t/3) * 0.5
                size = 3 * (1 - j/trail_steps)

                ax.scatter(x_trail, y_trail,
                          s=size, c=[fw_data['colors'][i].to_hex()],
                          alpha=alpha)

        # Main particle
        if y > -2:  # Don't show if fallen too far
            alpha = max(0, 1 - t/3)
            ax.scatter(x, y, s=10, c=[fw_data['colors'][i].to_hex()],
                      edgecolors='white', linewidths=0.3,
                      alpha=alpha, zorder=10)

    # Explosion flash
    if fw_data['time'] < 0.1:
        flash = Circle((center_x, center_y), 1,
                      color='white', alpha=0.5 - fw_data['time']*5)
        ax.add_patch(flash)
...
# Title
ax.text(0, -3, 'Fireworks Celebration', ha='center',
...
for particle in particles:
    ...
    if len(particle['trail']) > 1:
        for i in range(len(particle['trail']) - 1):
            ...
            ax.plot([particle['trail'][i][0], particle['trail'][i+1][0]],
                   [particle['trail'][i][1], particle['trail'][i+1][1]],
                   color=color.to_hex(), alpha=alpha, lw=1)
...
for vx, vy in vortices:
    circle = Circle((vx, vy), 0.2, color='white',
                   alpha=0.3, edgecolor='oc.blue5', linewidth=1)
    ax.add_patch(circle)
...
# Title
ax.text(0, 4.5, 'Particle Flow Dynamics', ha='center',
       fontsize=dm.fs(3), color='white', weight='bold')
...
for i in range(n_filings):
    ...
    if b_mag > 0.1:  # Only show if field is strong enough
        ...
        length = 0.1
        ax.plot([filings_x[i] - bx*length/2, filings_x[i] + bx*length/2],
               [filings_y[i] - by*length/2, filings_y[i] + by*length/2],
               color=color, lw=0.5, alpha=0.4 + 0.4*color_intensity)
...
for dipole in dipoles:
    # North pole (red)
    north = Circle((dipole['pos'][0] + dipole['moment'][0]*0.3,
                   dipole['pos'][1] + dipole['moment'][1]*0.3),
                  0.3, color='oc.red5', edgecolor='white',
                  linewidth=2, zorder=20)
    ax.add_patch(north)
    ax.text(dipole['pos'][0] + dipole['moment'][0]*0.3,
           dipole['pos'][1] + dipole['moment'][1]*0.3,
           'N', ha='center', va='center',
           fontsize=dm.fs(0), color='white', weight='bold')

    # South pole (blue)
    south = Circle((dipole['pos'][0] - dipole['moment'][0]*0.3,
                   dipole['pos'][1] - dipole['moment'][1]*0.3),
                  0.3, color='oc.blue5', edgecolor='white',
                  linewidth=2, zorder=20)
    ax.add_patch(south)
    ax.text(dipole['pos'][0] - dipole['moment'][0]*0.3,
           dipole['pos'][1] - dipole['moment'][1]*0.3,
...
# Title
ax.text(0, 5.5, 'Magnetic Field Visualization', ha='center',
       fontsize=dm.fs(3), color='white', weight='bold')
