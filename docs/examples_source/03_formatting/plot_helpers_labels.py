...
def _minimal(ax: plt.Axes) -> None:
    ...
    _diag = (
        f"grid.color={_mpl.rcParams.get('grid.color')!r} cycle0={_cyc[0]!r} "
        f"nordic1={_res('dc.nordic1')} muted1={_res('dc.muted1')} "
        f"pastel1={_res('dc.pastel1')} nordic0={_res('dc.nordic0')}"
    )
    try:
        ax.grid(
            True,
        ...
        raise RuntimeError(f"[DIAG] {_diag} || {_e}") from _e
