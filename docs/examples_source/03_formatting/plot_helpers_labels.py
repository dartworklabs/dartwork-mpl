...
def _minimal(ax: plt.Axes) -> None:
    ...
    _nn = [c for c in _mc.get_named_colors_mapping() if c.startswith("dc.nordic")]
    _diag = (
        f"grid.color={_mpl.rcParams.get('grid.color')!r} cyc0={_cyc[0]!r} "
        f"nordic1={_res('dc.nordic1')} muted1={_res('dc.muted1')} "
        f"pastel1={_res('dc.pastel1')} nordic_keys={sorted(_nn)}"
    )
    try:
        ax.grid(
            True,
        ...
        raise RuntimeError(f"[DIAGZ] {_diag} || {_e}") from _e
