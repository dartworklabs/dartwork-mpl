...
bars2 = ax.bar(x + width / 2, latency, width, label="Latency (ms)",
...
ax.bar(x - w, dataset_a, w, label="Dataset A", color="oc.blue3",
       edgecolor="white", linewidth=0.3)
ax.bar(x, dataset_b, w, label="Dataset B", color="oc.blue5",
       edgecolor="white", linewidth=0.3)
ax.bar(x + w, dataset_c, w, label="Dataset C", color="oc.blue7",
       edgecolor="white", linewidth=0.3)
...
for i in range(len(optimizers)):
    ...
    ax.text(
        x[i] + w, dataset_c[i], f"+{improvement:.1f}",
        ha="center", va="bottom",
