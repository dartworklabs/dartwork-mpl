...
for sigma, alpha in [(3, 0.1), (2, 0.2), (1, 0.35)]:
    ax.fill_between(x_test, 
                    ...
                    mean_pred + sigma * std_pred, 
                    color=base_color.to_hex(), alpha=alpha, lw=0,
                    label=f'{sigma}σ Credible Interval' if sigma == 3 else "")

# 2. Draw sample functions from the posterior
for i in range(4):
    sample_path = mean_pred + np.random.normal(0, 1) * std_pred * np.sin(x_test * np.random.uniform(2, 4))
    ax.plot(x_test, sample_path, color=base_color.to_hex(), alpha=0.25, lw=dm.lw(0.5))

# 3. Plot predictive mean
ax.plot(x_test, mean_pred, color='oc.indigo7', lw=dm.lw(1.5), label="Posterior Mean")

# 4. Plot training observations
ax.scatter(x_train, y_train, color='oc.red7', s=40, zorder=5, 
           edgecolors='white', linewidths=1.5, label="Observations")

ax.set_title("Gaussian Process Posterior Predictive", fontsize=dm.fs(1.5), weight='bold', pad=20)
