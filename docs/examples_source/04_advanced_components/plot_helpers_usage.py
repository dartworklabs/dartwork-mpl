...
print(f"X: {x_raw}, has NaN: {np.isnan(x_raw).any()}, has Inf: {np.isinf(x_raw).any()}")
print(f"Y: {y_raw}, has NaN: {np.isnan(y_raw).any()}, has Inf: {np.isinf(y_raw).any()}")
...
ax1.scatter(x_raw, y_raw, color='oc.red5', s=50, alpha=0.6, label='Raw (with NaN/Inf)')
...
# Cleaned data
ax2.scatter(x_clean, y_clean, color='oc.green5', s=50, alpha=0.6, label='Cleaned')
...
for i, color in enumerate(colors_qual):
    ...
    bar = ax1.bar(i, height, color=color,
...
ax4.text(0.05, 0.95, f'y = {z[0]:.2f}x + {z[1]:.2f}',
        transform=ax4.transAxes, fontsize=dm.fs(-1),
...
# Run the AI workflow
fig_ai, quality_issues = ai_create_visualization(test_data, chart_request="auto")
