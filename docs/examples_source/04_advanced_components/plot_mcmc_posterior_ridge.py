...
fig, axes = plt.subplots(n_params, 1, figsize=(dm.SW * 1.2, dm.SW * 1.5), sharex=True)
fig.subplots_adjust(hspace=-0.25)  # Negative hspace for the overlapping ridgeline effect

x_eval = np.linspace(-5, 15, 300)

for i, (ax, trace, color, label) in enumerate(zip(axes, traces, ridge_colors, parameters)):
    ...
    ax.fill_between(x_eval, 0, density_scaled, color=color.to_hex(), alpha=0.3, zorder=4)
    
    # Fill the HDI bounds heavily
    hdi_mask = (x_eval >= lo) & (x_eval <= hi)
    ax.fill_between(x_eval, 0, density_scaled, where=hdi_mask, 
                    color=color.to_hex(), alpha=0.85, zorder=4)
    
    # Cleanup individual axes
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.set_yticks([])
    ax.set_ylabel(label, rotation=0, labelpad=20, ha='right', va='center',
                  fontsize=dm.fs(0.5), weight='bold', color='oc.gray8')
...
axes[-1].set_xlabel("Effect Value (Standardized)", fontsize=dm.fs(0), labelpad=10)

fig.suptitle("Hierarchical Model Posteriors (94% HDI)", 
             fontsize=dm.fs(1.5), weight='bold', y=0.96)
