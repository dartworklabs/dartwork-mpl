...
# Title
fig.suptitle('Q2 2024 Financial Performance Dashboard',
             fontsize=dm.fs(2), weight='bold', y=0.98)
...
width = 0.35
ax1.bar(x - width/2, revenue, width, label='Revenue',
        color='oc.blue5', alpha=0.8)
ax1.bar(x + width/2, profit, width, label='Profit',
        color='oc.green5', alpha=0.8)

# Growth rate line
ax1_twin = ax1.twinx()
ax1_twin.plot(x, revenue_growth, 'o-', color='oc.orange6',
              linewidth=dm.lw(1), markersize=5, label='Growth %')
...
for metric, value in metrics.items():
    ax2.text(0.1, y_pos, metric + ':', fontsize=dm.fs(0),
             weight='bold', ha='left')
    ax2.text(0.9, y_pos, value, fontsize=dm.fs(0),
             ha='right', color='oc.green7' if '+' not in value else 'black')
    y_pos -= 0.2
...
for i, product in enumerate(products):
    ax3.fill_between(x, bottom, bottom + product_revenue[:, i],
                    alpha=0.7, color=colors[i], label=product)
...
ax4.plot(x, profit_margin, 'o-', color='oc.green6',
         linewidth=dm.lw(1.5), markersize=6)
...
regional_data = np.random.dirichlet(np.array([4, 3, 2, 1]), n_quarters).T * revenue
...
for i, quarter in enumerate(quarters[-3:]):  # Last 3 quarters
    ...
    ax5.bar(x_regional + offset, regional_data[:, -(3-i)],
           width_regional, label=quarter, alpha=0.8)

ax5.set_xlabel('Region', fontsize=dm.fs(0))
ax5.set_ylabel('Revenue ($M)', fontsize=dm.fs(0))
ax5.set_title('Regional Revenue Comparison (Last 3 Quarters)',
