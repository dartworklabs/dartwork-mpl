...
ax.plot(x, np.sin(x), color=base_color,
        label='Base (100%)', lw=dm.lw(1))
...
ax.plot(x, np.sin(x) - 0.5, color=color_50,
        label='Pseudo Alpha (50%)', lw=dm.lw(1))

# Pseudo alpha 15% — even lighter solid color
color_15 = dm.pseudo_alpha(base_color, 0.15, background='white')
ax.plot(x, np.sin(x) - 1.0, color=color_15,
        label='Pseudo Alpha (15%)', lw=dm.lw(1))
