...
gs = fig.add_gridspec(
    2, 2, left=0.12, right=0.96, top=0.94, bottom=0.10, wspace=0.4, hspace=0.5
)
...
ax_a.plot(
    t, signal + noise, color="oc.gray5", lw=0.3, label="Measured", alpha=0.7
)
...
bars = ax_c.bar(
    methods, accuracy, color=colors, edgecolor="white", linewidth=0.3, width=0.6
)
...
ax_d.hist(
    data_a,
    bins=25,
    color=dm.pseudo_alpha("oc.blue5", 0.5),
    edgecolor="oc.blue6",
    linewidth=0.3,
    label="Control",
)
ax_d.hist(
    data_b,
    bins=25,
    color=dm.pseudo_alpha("oc.red5", 0.5),
    edgecolor="oc.red6",
    linewidth=0.3,
    label="Treatment",
)
...
# Add panel labels
dm.label_axes([ax_a, ax_b, ax_c, ax_d], fontsize=dm.fs(1), fontweight="bold")
