...
ax.set_title(
    "Font Size Scaling: dm.fs(n)", fontsize=dm.fs(1), fontweight="bold"
)
...
ax.set_title(
    "Font Weight Scaling: dm.fw(n)", fontsize=dm.fs(1), fontweight="bold"
)
...
for i, (n, c) in enumerate(zip(line_offsets, colors, strict=False)):
    ...
    ax.plot(x, y, color=c, lw=dm.lw(n), label=f"lw({n:+.1f}) = {dm.lw(n):.2f}")

ax.set_title(
    "Line Width Scaling: dm.lw(n)", fontsize=dm.fs(1), fontweight="bold"
)
ax.set_xlabel("x", fontsize=dm.fs(0))
