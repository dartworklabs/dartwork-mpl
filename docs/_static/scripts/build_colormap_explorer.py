...
computes the 46-map payload from the color SSOT and injects it into a
self-contained HTML fragment (scoped ``#dm-cmap-exp`` CSS on ``--dm-*``
design tokens + vanilla JS), embedded by ``docs/color_system/colormaps.md``
via MyST ``{raw} html :file:``.
...
# ---------------------------------------------------------------------------
# Fragment: scoped #dm-cmap-exp CSS on --dm-* tokens + vanilla JS. Colours come
# only from __PAYLOAD__; demo geometry is generated in JS. Every CSS colour
# literal is a var(--dm-token,#fallback) fallback (no raw hex outside those).
# ---------------------------------------------------------------------------
...
<div id="dm-cmap-exp" class="dm-wide yue">
<style>
#dm-cmap-exp * {box-sizing:border-box;}
#dm-cmap-exp {width:100%;max-width:100%;overflow:clip;background:var(--dm-bg-page,#fff);color:var(--dm-gray-12,#1f2933);
  font-family:var(--dm-f-sys,"Inter",system-ui,sans-serif);letter-spacing:normal;}
#dm-cmap-exp .cx-count {font:600 12px var(--dm-f-mono,ui-monospace,monospace);color:var(--dm-text-muted,#667085);margin:0 0 16px;}
#dm-cmap-exp .md {display:grid;grid-template-columns:minmax(10.5rem,12rem) minmax(0,1fr);gap:var(--dm-space-5,24px);align-items:start;}
@media(max-width:760px){#dm-cmap-exp .md{grid-template-columns:1fr;gap:var(--dm-space-4,16px);}#dm-cmap-exp .detail{position:static!important;}}
#dm-cmap-exp .rail {display:flex;flex-direction:column;gap:1px;position:sticky;top:96px;
  max-height:calc(100vh - 120px);overflow-y:auto;padding-right:8px;scrollbar-width:thin;}
#dm-cmap-exp .rail::-webkit-scrollbar{width:9px;}
#dm-cmap-exp .rail::-webkit-scrollbar-thumb{background:var(--dm-gray-a4,rgba(0,0,0,.13));border-radius:9px;border:2px solid transparent;background-clip:content-box;}
#dm-cmap-exp .rail .fh {font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.05em;
  color:var(--dm-gray-10,#7c8794);margin:16px 0 5px;}
#dm-cmap-exp .rail .fh:first-child {margin-top:0;}
#dm-cmap-exp .ri {display:flex;align-items:center;gap:10px;padding:4px 9px;border-radius:var(--dm-radius-4,8px);cursor:pointer;min-width:0;border:0;background:transparent;text-align:left;color:inherit;width:100%;}
#dm-cmap-exp .ri:hover {background:var(--dm-gray-a2,rgba(0,0,0,.05));}
#dm-cmap-exp .ri.on {background:var(--dm-accent-2,#e6f7f4);}
#dm-cmap-exp .ri .mini {flex:0 0 40px;height:12px;}
#dm-cmap-exp .ri .nm {font-size:12px;font-weight:400;color:var(--dm-gray-12,#1f2933);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;}
#dm-cmap-exp .ri.on .nm {color:var(--dm-accent-11,#0c6b5e);font-weight:500;}
#dm-cmap-exp .detail {min-width:0;background:var(--dm-bg-panel,#f7f9f9);border:1px solid var(--dm-border-faint,rgba(0,0,0,.12));border-radius:var(--dm-radius-5,12px);padding:var(--dm-space-5,24px);}
#dm-cmap-exp .d-ey {font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:var(--dm-accent-11,#0c6b5e);margin-bottom:7px;}
#dm-cmap-exp .d-title {display:flex;align-items:baseline;gap:11px;flex-wrap:wrap;}
#dm-cmap-exp .d-title h3 {margin:0;font-size:var(--dm-type-display-size,25px);line-height:var(--dm-type-display-line,1.2);font-weight:var(--dm-type-display-weight,700);letter-spacing:var(--dm-type-display-spacing,-.02em);}
#dm-cmap-exp .d-key {font-family:var(--dm-f-mono,monospace);font-size:13px;color:var(--dm-accent-11,#0c6b5e);
...
#dm-cmap-exp .d-key:hover {background:var(--dm-accent-3,#d3f1ea);}
#dm-cmap-exp .d-key.copied {background:var(--dm-accent-3,#e0f8f3);color:var(--dm-accent-11,#008573);}
#dm-cmap-exp .d-key.copied::after {content:" \2713";}
#dm-cmap-exp .d-use {font-size:15.5px;color:var(--dm-gray-11,#4a5560);line-height:1.62;margin:11px 0 8px;min-height:3.24em;}
#dm-cmap-exp .a11y-chips {display:flex;flex-wrap:wrap;gap:7px;margin-left:auto;align-items:center;justify-content:flex-end;align-self:center;}
#dm-cmap-exp .a11y-chip {--a-color:var(--dm-gray-8,#98a1ab);position:relative;display:inline-flex;align-items:center;gap:6px;border:1px solid var(--a-color);
...
#dm-cmap-exp .a11y-chip.ok {--a-color:var(--dm-accent-9,#12a594);}
#dm-cmap-exp .a11y-chip.mid {--a-color:var(--dm-warning,#d97706);}
#dm-cmap-exp .a11y-chip.bad {--a-color:var(--dm-danger,#dc2626);}
#dm-cmap-exp .a11y-chip.info {--a-color:var(--dm-gray-8,#64748b);}
#dm-cmap-exp .a-dot {width:8px;height:8px;border-radius:var(--dm-radius-full,999px);background:var(--a-color);flex:0 0 8px;}
#dm-cmap-exp .a-label {font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--dm-gray-9,#8794a1);}
#dm-cmap-exp .a-num {font-family:var(--dm-f-mono,monospace);font-size:11px;color:var(--dm-gray-11,#4a5560);}
#dm-cmap-exp .a11y-chip::after {content:attr(data-tip);position:absolute;right:0;top:calc(100% + 8px);width:max-content;max-width:min(320px,calc(100vw - 32px));
...
#dm-cmap-exp .a11y-chip:hover::after,#dm-cmap-exp .a11y-chip:focus::after {opacity:1;visibility:visible;transform:translateY(0);}
@media(prefers-reduced-motion:reduce){#dm-cmap-exp .a11y-chip::after{transition:none;}}
@media(max-width:560px){#dm-cmap-exp .a11y-chips{margin-left:0;flex-basis:100%;justify-content:flex-start;}}
#dm-cmap-exp .d-bar {display:flex;align-items:center;gap:13px;flex-wrap:wrap;row-gap:8px;margin-bottom:14px;}
#dm-cmap-exp .field {display:inline-flex;align-items:center;gap:8px;flex:0 0 auto;}
#dm-cmap-exp .cl {font-size:13px;font-weight:500;color:var(--dm-gray-9,#8794a1);letter-spacing:normal;text-transform:none;white-space:nowrap;}
#dm-cmap-exp .crng {accent-color:var(--dm-accent-9,#12a594);width:150px;}
#dm-cmap-exp .cval {font-size:13px;font-weight:700;color:var(--dm-accent-11,#0c6b5e);min-width:7ch;font-family:var(--dm-f-mono,monospace);}
#dm-cmap-exp .tgl {appearance:none;border:0;background:transparent;padding:0;cursor:pointer;display:inline-flex;align-items:center;gap:6px;font:inherit;flex:0 0 auto;white-space:nowrap;}
#dm-cmap-exp .tgl .tgl-l {font-size:13px;font-weight:500;color:var(--dm-gray-9,#8794a1);transition:color .12s;}
#dm-cmap-exp .tgl.on .tgl-l {color:var(--dm-gray-12,#1f2933);}
#dm-cmap-exp .tgl-tr {width:32px;height:18px;border-radius:var(--dm-radius-full,999px);background:var(--dm-gray-5,#c3c9d0);position:relative;transition:background .15s;flex:0 0 auto;}
#dm-cmap-exp .tgl-kn {position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:var(--dm-radius-full,999px);background:var(--dm-bg-page,#fff);box-shadow:0 1px 2px rgba(0,0,0,.28);transition:left .15s;}
#dm-cmap-exp .tgl.on .tgl-tr {background:var(--dm-accent-9,#12a594);}
#dm-cmap-exp .tgl.on .tgl-kn {left:16px;}
#dm-cmap-exp .seg {display:inline-flex;align-items:center;border:1px solid var(--dm-border-faint,rgba(0,0,0,.12));border-radius:var(--dm-radius-full,999px);overflow:hidden;background:var(--dm-bg-page,#fff);}
#dm-cmap-exp .seg button {appearance:none;border:0;border-right:1px solid var(--dm-border-faint,rgba(0,0,0,.12));background:transparent;color:var(--dm-gray-10,#667085);padding:5px 9px;font:650 11px var(--dm-f-mono,ui-monospace,monospace);cursor:pointer;white-space:nowrap;}
#dm-cmap-exp .seg button:last-child {border-right:0;}
#dm-cmap-exp .seg button.on {background:var(--dm-accent-2,#e6f7f4);color:var(--dm-accent-11,#0c6b5e);}
#dm-cmap-exp .demo-tools {display:flex;align-items:flex-start;gap:13px;flex-wrap:wrap;row-gap:8px;border-top:1px solid var(--dm-border-faint,rgba(0,0,0,.12));padding-top:12px;margin-bottom:14px;}
...
#dm-cmap-exp .demo-chip svg {width:18px;height:14px;}
#dm-cmap-exp .demo-chip.on {border-color:var(--dm-accent-8,#57c7b6);background:var(--dm-accent-2,#e6f7f4);color:var(--dm-accent-11,#0c6b5e);}
...
#dm-cmap-exp .demo-grid {display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;}
#dm-cmap-exp .demo-grid.layout-4,#dm-cmap-exp .demo-grid.layout-6 {grid-template-columns:repeat(2,minmax(0,1fr));}
@media(max-width:1040px){#dm-cmap-exp .demo-grid{grid-template-columns:repeat(2,minmax(0,1fr));}}
@media(max-width:680px){#dm-cmap-exp .demo-grid,#dm-cmap-exp .demo-grid.layout-4,#dm-cmap-exp .demo-grid.layout-6{grid-template-columns:1fr;}}
#dm-cmap-exp .demo-card {position:relative;background:var(--dm-bg-page,#fff);border:1px solid var(--dm-border-faint,rgba(0,0,0,.12));border-radius:var(--dm-radius-4,8px);aspect-ratio:16/10;overflow:hidden;}
#dm-cmap-exp .demo-flex {position:absolute;inset:0;display:flex;align-items:center;justify-content:center;}
#dm-cmap-exp .demo-label {position:absolute;left:8px;top:6px;z-index:2;font:700 10px var(--dm-f-mono,ui-monospace,monospace);color:var(--dm-gray-12,#1f2933);letter-spacing:0;background:color-mix(in srgb,var(--dm-bg-page,#fff) 82%,transparent);border-radius:var(--dm-radius-full,999px);padding:2px 6px;backdrop-filter:blur(3px);box-shadow:0 1px 4px rgba(0,0,0,.13);}
#dm-cmap-exp svg.demo-svg {display:block;width:100%;height:100%;}
#dm-cmap-exp canvas.demo-canvas {display:block;width:100%;height:100%;image-rendering:auto;}
#dm-cmap-exp .demo-grid.gs svg.demo-svg,#dm-cmap-exp .demo-grid.gs canvas.demo-canvas {filter:grayscale(1);}
#dm-cmap-exp .code {margin:16px 0 14px;background:var(--dm-i-code-surface,#f2f4f5);border-radius:var(--dm-radius-4,8px);padding:11px 14px;
  font-family:var(--dm-f-mono,monospace);font-size:11.5px;color:var(--dm-gray-12,#1f2933);white-space:pre;overflow-x:auto;}
#dm-cmap-exp .code pre {margin:0;padding:0;background:transparent;font:inherit;color:inherit;white-space:pre;}
#dm-cmap-exp .code .c1{color:var(--dm-gray-10,#697b85);}
#dm-cmap-exp .code .s1,#dm-cmap-exp .code .s2{color:var(--dm-accent-11,#0c8f7f);}
#dm-cmap-exp .code .kn,#dm-cmap-exp .code .k{color:var(--dm-code-kw,#7c3aed);font-weight:650;}
#dm-cmap-exp .code .mi{color:var(--dm-warning,#b45309);}
#dm-cmap-exp .meta {font-size:13px;color:var(--dm-text-muted,#667085);line-height:1.6;display:grid;gap:3px;margin-top:2px;border-top:1px solid var(--dm-border-faint,rgba(0,0,0,.12));padding-top:12px;}
#dm-cmap-exp .meta .m-l {color:var(--dm-gray-11,#4a5560);font-weight:650;}
#dm-cmap-exp .dm-toast {position:fixed;bottom:26px;left:50%;transform:translateX(-50%);background:var(--dm-gray-12,#1f2933);color:var(--dm-bg-page,#fff);padding:8px 16px;border-radius:var(--dm-radius-full,999px);font-size:12.5px;opacity:0;transition:opacity .18s;pointer-events:none;z-index:99999;}
#dm-cmap-exp .dm-toast.show {opacity:1;}
</style>
