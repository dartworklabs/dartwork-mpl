...
The colormap surface is the Model B catalog: **43 continuous colormaps**, their
`_r` reverses, and **13 qualitative colormaps** registered for `cmap=`
interfaces — `dm.list_colors()` returns the 56 Model B family records. Every
continuous map is generated by the same perceptual recipe as the `dc.*` palette
— designed on CIELAB L\* and OKLCH, equalized in OKLab ΔE, and checked against
hard gates before it ships. The generation axioms, metric system, naming
grammar, and benchmarks live in [Design rationale](design-rationale.md).
