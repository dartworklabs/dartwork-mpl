...
| Reaching for hex codes              | `color="dc.corporate3"` (curated 8-mood palette), plus `"oc.*"`, `"tw.*"`, `"md.*"`, `"ad.*"`, `"cu.*"`, `"pr.*"` for third-party systems |
...
ax.plot(x, np.sin(x), color="dc.corporate3", label="signal", lw=dm.lw(1.5))
...
ax.plot(x, y, color="dc.corporate3")                   # cool blue
ax.fill_between(x, y1, y2, color="dc.earth1")     # warm accent
ax.bar(categories, values, color="dc.spectrum1")      # bold call-out

# Third-party systems also available for cross-team consistency
ax.plot(x, y2, color="dc.corporate2")                   # OpenColor
...
ax1.plot(x, np.sin(x), color="dc.spectrum1")
ax2.plot(x, np.cos(x), color="dc.corporate3")
