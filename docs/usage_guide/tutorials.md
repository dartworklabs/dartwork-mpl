...
ax1.plot(t, signal, color="dc.teal3", lw=dm.lw(1))
ax1.fill_between(t, signal - 0.15, signal + 0.15,
                 color="dc.teal1", alpha=0.4)
...
ax2.semilogy(freq[:50], spectrum[:50], color="dc.red2", lw=dm.lw(1))
...
bars = ax.bar(weeks, throughput, color="dc.teal2", width=0.6, zorder=2)
ax.set_ylabel("처리량 (건)")
ax.yaxis.set_major_formatter(mticker.StrMethodFormatter("{x:,.0f}"))

# 5. Line chart for the secondary series on a twin axis
ax2 = ax.twinx()
ax2.plot(weeks, utilization, color="dc.red2", marker="o",
