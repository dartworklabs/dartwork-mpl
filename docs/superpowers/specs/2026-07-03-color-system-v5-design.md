...
| 등화 균일성 | 이웃 ΔE 변동계수 cv ≤ 0.08 (팔레트 사다리 한정 — seq-cmap cv는 산출만 하고 하드 게이트하지 않는다; 호장 등화가 구조적으로 안전) | OKLab ΔE |
| categorical 접근성 | 흔한 색각이상(정상+protan+deutan) ΔE00 ≥ 10 ∧ 희귀 tritan(Brettel-1997) ≥ 8 | CIEDE2000 |
...
| **접근성 게이트** | CIEDE2000 | 산업 표준 판별 지표. Okabe-Ito 벤치마크(공통 min ΔE00 11.5, BVM tritan 기준)로 임계 캘리브레이션 |
...
> 사다리 중앙 hue `h_m = h₀ + Δh·0.5^γ`에서 평가해 같은 그리드로 반올림한 값이며, 신규 family를 추가할 때는 `h₀`·`Δh`·`γ`·`t_p`
...
matplotlib 대표 멀티휴 3종과의 벤치마크 — **측정 프로토콜을 명시한다**: 양쪽 모두 출하 256-LUT를
32-stop으로 샘플링하고 이웃 OKLab ΔE의 변동계수(cv)로 잰다(동일 조건 — 사용자가 재현하는 바로 그
LUT. 레시피의 직접 32-stop 렌더는 별도이며 균일성이 더 높게 나온다). L\* 단조는 두 계열 모두
통과하므로 표에서 생략한다.

| 비교쌍 | ΔE cv (낮을수록 균일) | L\* 범위 |
|---|---|---|
| **`aurora`** vs viridis | **0.063** vs 0.086 | **81.9** vs 76.0 |
| `afterglow` vs plasma | **0.050** vs 0.082 | 76.1 vs 78.9 |
| `blaze` vs magma | **0.064** vs 0.201 | 82.1 vs 97.8 |

aurora는 viridis 대비 균일성 ~1.3배·L\* 범위 +5.9로 기본값 자격이 있다. magma의 넓은 L\* 범위(97.8)는
