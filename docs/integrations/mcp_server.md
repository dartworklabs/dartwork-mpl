...
- _"Use the `dartwork-mpl` MCP server to give me the hex code for `dc.corporate2`."_ → triggers `get_color_value`.
...
3. **Live color lookup**
   - **You ask:** _"What's the hex code for dc.corporate2?"_
   - **MCP in action:** The assistant calls `get_color_value("dc.corporate2")` and returns the exact hex code — no guessing.
