...
def plot_signal(
    ...
    data : np.ndarray
        Signal data to plot.
    color : str, optional
        Line color. Default is 'dc.corporate2'.
...
fig, ax = plot_signal(data, color='dc.spectrum2', linewidth=1.2)
dm.save_and_show(fig)
```

**Why this works better with AI:**

- "Change the line color to `dc.spectrum2`" → AI modifies one argument
...
| ------------------------------------------ | ------------------------------ |
| "Change color to `dc.spectrum2`"                | "Make it a bit redder"         |
...
def plot_comparison(
    data1: np.ndarray,
    data2: np.ndarray,
    color1: str = 'dc.spectrum2',
    color2: str = 'dc.corporate2',
    linewidth: float = 0.7,
    title: str | None = None,
) -> tuple[plt.Figure, plt.Axes]:
