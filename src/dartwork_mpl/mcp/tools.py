...
def register_tools(mcp: FastMCP) -> None:
    ...
    def list_color_families() -> str:
        ...
        for prefix in prefixes:
            colors = {k: mcolors.to_hex(v) for k, v in mapping.items() if k.startswith(prefix)}
            if colors:
    ...
    def lint_dartwork_mpl_code(code: str) -> str:
        ...
        if "plt.show()" not in code and "savefig" not in code and "dm.show(" not in code and "dm.save_and_show(" not in code:
            issues.append(
        ...
        if not issues:
            return "✅ No issues found. Code follows dartwork-mpl best practices."
        return "\n".join(issues)
    ...
    def dartwork_mpl_info() -> str:
        ...
        return json.dumps({
            ...
            "style_presets": [
                "base", "dmpl", "dmpl_light", "font-minimal",
                "font-poster", "font-presentation", "font-report",
                "font-scientific", "font-web", "lang-kr",
                "spine-no", "spine-yes", "theme-dark", "theme-minimal",
            ],
            "plot_templates": [
                "tornado", "scatter", "bar", "heatmap", "line", "violin",
                "stacked_bar", "boxplot", "pie", "histogram", "contour",
                "twin_axis",
            ],
        }, indent=2)
...
def _validate_tornado(data: dict) -> str:
    ...
    if "positive" not in data and "negative" not in data:
        issues.append("Need at least 'positive' and/or 'negative' value arrays.")
    if "categories" in data and "positive" in data:
        if len(data["categories"]) != len(data["positive"]):
            issues.append("Length mismatch: 'categories' and 'positive' must have equal length.")
    return "✅ Data structure valid for tornado plot." if not issues else "\n".join(issues)
...
def _validate_scatter(data: dict) -> str:
    ...
    return "✅ Data structure valid for scatter plot." if not issues else "\n".join(issues)
...
def _validate_bar(data: dict) -> str:
    ...
    if "categories" in data and "values" in data:
        if len(data["categories"]) != len(data["values"]):
            issues.append("Length mismatch: 'categories' and 'values' must have equal length.")
    return "✅ Data structure valid for bar plot." if not issues else "\n".join(issues)
...
def _validate_heatmap(data: dict) -> str:
    ...
    elif not isinstance(data["matrix"], list) or not all(isinstance(row, list) for row in data["matrix"]):
        issues.append("'matrix' must be a 2D array (list of lists).")
    return "✅ Data structure valid for heatmap." if not issues else "\n".join(issues)
...
def _validate_stacked_bar(data: dict) -> str:
    ...
    if "series" not in data:
        ...
        for name, vals in data["series"].items():
            if len(vals) != cat_len:
                issues.append(f"Series '{name}' length ({len(vals)}) != categories length ({cat_len}).")
    return "✅ Data structure valid for stacked bar." if not issues else "\n".join(issues)
...
def _validate_pie(data: dict) -> str:
    ...
    if "labels" in data and "sizes" in data:
        if len(data["labels"]) != len(data["sizes"]):
            issues.append("Length mismatch: 'labels' and 'sizes' must have equal length.")
    return "✅ Data structure valid for pie chart." if not issues else "\n".join(issues)
...
def _validate_line(data: dict) -> str:
    ...
    if "y" not in data and "series" not in data:
        issues.append("Need 'y' (single series) or 'series' (dict of named series).")
    return "✅ Data structure valid for line plot." if not issues else "\n".join(issues)
